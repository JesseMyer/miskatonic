...
# Vril Prompt — VRL-MA-COM-002 — Component — diagram

## Purpose

Invoke Vril to produce a **diagram** deliverable for M&A Integration Architecture work at
**Component** altitude. The output must be traceable to declared Ossuary patterns and must
...
- Relevant Ossuary pattern IDs at Component altitude (name them explicitly)
- Stakeholder quotes vs. document citations (dual-track when both exist)
- Known Bureau findings that constrain the answer

## Output Contract — diagram

Produce a diagram specification: nodes, edges, swimlanes, and legend. Prefer Mermaid when possible; if not, provide a rigorous textual graph. Declare what is unknown.
